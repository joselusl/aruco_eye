...
int main(int argc,char **argv)
{
	...
	try
	{
        ...
        //Start retina
        //ArucoRetina.start();
        const std::string arucoList="/home/joselusl/workspace/ros/pf_sensor_fusion_catkin_ws/src/fuseon_stack/packages/perception/aruco/aruco_eye/config/arUcoList.xml";
        const std::string cameraCalibration="/home/joselusl/workspace/ros/pf_sensor_fusion_catkin_ws/src/fuseon_stack/packages/perception/aruco/aruco_eye/config/camera.yml";
        ...
        while ( key!=27 && TheVideoCapturer.grab())
		{
            ...
            if(!ArucoRetina.getMarkersList(TheMarkers))
            {
            ...
            for(unsigned int i=0; i<TheMarkers.size();i++)
            {
                ...
                aruco::Marker TheMarker=TheMarkers[i].getMarker();

                cv::Mat HomogMat;

                homogeneusTransformation::createMatHomogFromVecs(HomogMat,TheMarker.Tvec,TheMarker.Rvec);
...
#ifdef DEBUG_ARUCO_EYE
                  //Sacamos valores
                std::cout<<"[TEST] ->Marker id="<<TheMarker.id<<endl;
                std::cout<<"[TEST]   Trans Vec="<<TheMarker.Tvec<<endl;
                std::cout<<"[TEST]   Rot Vec="<<TheMarker.Rvec<<endl;
            ...
            //Draw
            key=ArucoRetina.drawAndDisplayDetectedArucoCodes(windowName,waitTime,true,true);
